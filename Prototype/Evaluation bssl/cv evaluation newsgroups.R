...
# Data --------------------------------------------------------------------
load(file = "C:/Users/erogeka/Documents/Analysis/cv-ssl/newsgroup/newsdtm.rda")
...
Gibbs = LDA(news.dtm, k = 20, method = "Gibbs",
...
Gibbs@terms <- news.dtm@Dimnames$Docs
Gibbs@documents <- news.dtm@Dimnames$Terms
save(Gibbs, file = "C:/Users/erogeka/Documents/Analysis/cv-ssl/newsgroup/Gibbs-20.rda")


load("C:/Users/erogeka/Documents/Analysis/cv-ssl/newsgroup/Gibbs-20.rda")

x <- data.frame(Gibbs@gamma)
X <- model.matrix(~.,x)
y <- news.group
n <- length(y)
...
save(acc.prob, acc.max, mean.entropy, file='C:/Users/erogeka/Documents/Analysis/cv-ssl/result-newsgr.rda')
# load(file='C:/Users/erogeka/Documents/Analysis/cv-ssl/result-newsgr.rda')
...
ggsave(plot.a.prob, file='C:/Users/erogeka/Documents/Analysis/cv-ssl/aprob-newsgr.pdf',
       width=3, height=2, scale =2)
...
plot.a.max <- ggplot(acc.m.sum, aes(x=Labeled, y=Accuracy, colour=Model, group=Model)) + 
  geom_errorbar(aes(ymin=Accuracy-ci, ymax=Accuracy+ci), width=1,  position=pd) +
  geom_line(position=pd) + geom_point(position=pd) + theme_bw()
ggsave(plot.a.max, file='C:/Users/erogeka/Documents/Analysis/cv-ssl/amax-newsrg.pdf',
       width=3, height=2, scale =2)
...
plot.entropy <- ggplot(entr.sum, aes(x=Labeled, y=Entropy, colour=Model, group=Model)) + 
  ...
  geom_line(position=pd) + geom_point(position=pd) + theme_bw()
ggsave(plot.entropy, file='C:/Users/erogeka/Documents/Analysis/cv-ssl/entropy-newsgr.pdf',
       width=3, height=2, scale =2)


# Combined acc and entr for the report
plot.acc <- plot.a.max + theme(legend.position="none")
pdf('C:/Users/erogeka/Documents/Analysis/cv-ssl/combo-newsgr.pdf', width = 8, height = 3)
